...
const registerGovernment = async (userData) => {
  try {
    const response = await API.post("/auth/register/government", userData);
    return response.data;
  } catch (error) {
    // Capture detailed FastAPI validation errors
...
const RegisterGovernment = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await registerGovernment(formData);
      // ✅ Success - redirect to login page
      navigate('/');
    } catch (err) {
      // ✅ Handle FastAPI 422 errors or custom backend messages
      if (Array.isArray(err)) {
        // Extract all 'msg' fields from the error array
