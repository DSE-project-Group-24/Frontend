describe("Register Nurse Page", () => {
  beforeEach(() => {
    cy.visit("/register/nurse");
  });

  // ✅ Test Case 1: Check UI elements are visible
  it("should display all registration fields correctly", () => {
    cy.contains("Register as Nurse").should("be.visible");
  ...
  it("should show error when registration fails", () => {
    cy.intercept("POST", "**/auth/register/nurse", {
      statusCode: 400,
  ...
  it("should show loading text while registering", () => {
    cy.intercept("POST", "**/auth/register/nurse", (req) => {
      // Simulate delay
      req.reply((res) => {
        res.delay = 1000;
        res.send({ id: 1, name: "Jane Doe" });
      });
    }).as("slowRegister");
